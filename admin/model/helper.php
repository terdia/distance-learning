...
class helper{
            ...
            if($rs = $statement->fetch()){
                ...
                if($balance != ""){
                    ...
                    <input type="text" name="txtMatric" id="txtMatric"
                     size="50" value="u'.$matric_no.'" disabled="disabled" style="border: 0;"></td>
                    </tr>
                    <tr>
                 ...
                 <p><input id="p_info" type="button" name="parseSemester"
                    ...
                    Please contact finance department for clearance, student financial details could not be verified</p>";
                }
            }
        }
        return $output;
    ...
    public function loadAssignment(){
        if(isset($_SESSION['course']) && isset($_SESSION['adminPass'])){
            $lecturer_course = $_SESSION['course'];
        }
        $con = new data();
        $sql = "SELECT * FROM submission WHERE programme=:programme";
        $statement = $con->prepare($sql);
        $statement->execute(array(':programme'=> $lecturer_course));
        ...
        $sql = "SELECT * FROM submission WHERE programme=:programme $limit";
        $statement = $con->prepare($sql);
        $statement->execute(array(':programme'=> $lecturer_course));
        $textline2 = "Page <b>$pagenum</b> of <b>$last</b>";
        ...
        if($statement->rowCount() > 0){
            ...
            while($rs = $statement->fetch()){
                ...
                $submitID = $rs['submitID'];
                $submit_date = $rs['submit_date'];
                $semester = $rs['semester'];
            }
              $submit_date = strftime("%B %d, %Y %I:%M %p", strtotime($submit_date));
              $registrationConfirm = "SELECT * FROM semester_registration
                WHERE matric_no=:matric_no LIMIT 1";
                $statement = $con->prepare($registrationConfirm);
                $statement->execute(array('matric_no'=> $submitID));
                if($statement->rowCount() > 0){
                    ...
                    $breakApart = explode(',', $modules);
                    if(count($breakApart) > 0){
                        foreach($breakApart as $course){
                            $folder = "../submission/".$programme."/".$semester."/".$course."/";
                            if(file_exists($folder)){
                                $handle = opendir($folder);
                                while($files = readdir($handle)){
                                    if(!is_dir($files)){
                                        $output .= '<tr><td>'.$course.'</td>
                                        <td ><a href="'.$folder.$files.'">'.$files.'</a></td>
                                        <td class="hide">'.$submit_date.'</td>
                                        </tr>';
                                    }
                                }
                                closedir($handle);
    ...
    function loadExam($moduleName, $theStudentID){
        $con = new data();
        $stuID = "";

        $output = "";
        $sql = "SELECT * FROM answer
                WHERE modules='$moduleName' AND studentID='$theStudentID'";
        $statement = $con->query($sql);
        if($statement){
            while($rs = $statement->fetch()){
                $question1 = $rs['question1Answer'];
                $question2 = $rs['question2Answer'];
                $question3 = $rs['question3Answer'];
                $question5 = $rs['question5Answer'];
                $semester = $rs['semester'];
                $course = $rs['programme'];
                $modules = $rs['modules'];
                $examCode = $rs['studentID'];
                ...
                if($modules){
                    ...
                    <label class="enrollMobile">Answer for Question 1</label>
                     <textarea name="question1Answer" id="question1Answer" style="border:0; height:auto;" disabled="disabled">'.$question1.'</textarea></td>
                    </tr>
                    ...
                    <label class="enrollMobile">Answer for Question 2</label>
                     <textarea name="question2Answer" id="question2Answer" style="border:0; height:150px;" disabled="disabled">'.$question2.'</textarea></td>
                    </tr>
                    ...
                    <label class="enrollMobile">Answer for Question 3</label>
                     <textarea name="question3Answer" id="question3Answer" style="border:0; height:150px;" disabled="disabled">'.$question3.'</textarea></td>
                    </tr>';}
                ...
                if($question5){
                    $output .= '<tr>
                        ...
                        <label class="enrollMobile">Answer for Question 4</label>
                         <textarea name="question5Answer" id="question5Answer" style="border:0; height:150px;" disabled="disabled">'.$question5.'</textarea></td>
                        </tr>';
                }else{
            ...
            if($output != ""){
                $output .= '</table>
                    ...
                    <input name="txtModules" id="txtModules" type="hidden" value="'.$modules.'" > </p>
                </fieldset>
                <fieldset>
                        <legend><strong>Module Scores</strong></legend>
                        <table width="100%" border="0" cellpadding="1">
                            <tr>
                                <td class="enrollNormal enrollfirstTd">Enter Marks: *</td>
                                <td class="enrollsecondTd">
                                    <label class="enrollMobile">Enter Marks: *</label>
                                    Course Work <input style="width: 15%;" type="text" name="txtCourseWork" maxlength="3" id="txtCourseWork">
                                    Exam <input style="width: 15%;" type="text" name="txtExamMark" maxlength="3" id="txtExamMark">
                            </tr>
                            </table>
                   <p><input id="p_info" type="button" name="parseUploadResult"
                        style="float:right;" class="btn" value="Upload Result" onclick="parseResult()">
                    </p>
                        </fieldset>
                ';
            }
            else if(!$modules){
                $output = "<h5><strong>Sorry no record found</strong></h5>";
            }
        }
        return $output;
    }
