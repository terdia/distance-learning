...
//FUNCTIONS THAT ARE SPECIFIC TO STUDENT TABLE
    ...
    public function getProfilePic($matric){
        $con = new data();
        ...
        $check_pic = "students/$matric/$avatar";
        $default_pic = "students/default-no-profile-pic.jpg";

        if($avatar != ""){
            if(file_exists($check_pic)){
                $profile_pic = "<img src=".$check_pic." />";
                return $profile_pic;
            }
        }else{
            $profile_pic = "<img src=".$default_pic." />";
            return $profile_pic;
        }

    }

    public function changeProfileForm($theID){
        $avatar_form  = '<form id="avatar_form" enctype="multipart/form-data" method="post">';
        $avatar_form .=   '<h4>Change Picture</h4><br/>';
        $avatar_form .=   '<input type="file" name="avatar" required>';
        $avatar_form .=   '<input type="hidden" name="matric_no" value="'.$theID.'">';
    ...
    public function profileBtn(){
        $profile_pic_btn = '<a href="#" onclick="return false;" onmousedown="toggleElement(\'avatar_form\')"><img src="images/editprofilepic.png"> Toggle To Edit Profile Picture</a>';
        return $profile_pic_btn;
    }

    public function loadClassContent($thID){
        $output = "";
        $week = "";
        $video = "";
        if(isset($_GET['coursenameToLoad'])){
            $fname = $_GET['coursenameToLoad'];
        }
        else{
            $fname = "materials";
        }
        ...
        $statement = $con->prepare($sql);
        $statement->execute(array('matric_no'=> $thID));
        ...
        $sqlModule = "SELECT modulename, week
                      FROM coursematerial
                      WHERE programme=:programme
                      AND semester=:semester";
        $stat = $con->prepare($sqlModule);
        $stat->execute(array('programme'=> $programme, 'semester'=>$semester));
        if($stat->rowCount() > 0){
        ...
        if(count($breakApart) > 0){
            $video .= "<h5>Module Videos</h5><div class=\"video_gallery_container\">";
            foreach($breakApart as $course){
                $folder = $fname."/".$programme."/".$semester."/".$course."/".$week."/";
               if(file_exists($folder)){
                   ...
                   while($files = readdir($handle)){
                       ...
                       if(!is_dir($files) && strpos($files, ".pdf")){
                           $output .= $course."<br>";
                           $output .= '<a target="_blank" href="'.$folder.$files.'">View Content -> '.$files.'</a><br>';
                       }
                       else if(!is_dir($files) && strpos($files, ".mp4")){
                           $video .= "<a class=\"videoLink\" path=\"$folder\" videowidth=\"800\"
                            videoheight=\"450\" videofile=\"$jpeg\"
                            videocaption=\"$course\" ></a>";
                       }
                   }
                   closedir($handle);
               }
            ...
            return "You have not been registered for the current semester";
        }
        $video .= "<div class=\"clear_both\"></div></div>";
        return $output.$video;
    }
    ...
    function getComment(){
        ...
        if($statement->rowCount() > 0){
            ...
            while($row = $statement->fetch()){
                ...
                if($postUserid == $_SESSION['userMatricNo']){
    ...
    public function loadAssignment($thID){
        $output = "";
        if(isset($_GET['coursenameToLoad'])){
            $fname = $_GET['coursenameToLoad'];
        }
        ...
        if($statement->rowCount() > 0){
            ...
            $breakApart = explode(',', $modules);
            $sqlModule = "SELECT modulename, topic
                      FROM assignments
                      WHERE programme=:programme
                      AND semester=:semester";
            $stat = $con->prepare($sqlModule);
            ...
            if(count($breakApart) > 0){
                foreach($breakApart as $course){
                    $folder = $fname."/".$programme."/".$semester."/".$course."/";
                    if(file_exists($folder)){
                        $handle = opendir($folder);
                        while($files = readdir($handle)){
                            if(!is_dir($files)){
                                $output .= $course."<br>";
                                $output .= '<a href="'.$folder.$files.'">Download -> '.$files.'</a><br>';
                            }
                        }
                        closedir($handle);
                    }
